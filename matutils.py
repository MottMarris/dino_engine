...
# Translates object back to the origin, rotates by the desired axis it then puts in back
# in its original place

def rotateInPlace(angle, position, axis):

    back = translationMatrix(position)

    for i in range(len(position)):
        position[i] = -position[i]
    origin = translationMatrix(position)
    ...
    x = np.matmul(back,np.matmul(rotation,origin)) 
