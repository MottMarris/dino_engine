...
class BaseModel:
    ...
    def __init__(self, scene, M=poseMatrix(), mesh=Mesh(), color=[1., 1., 1.], primitive=GL_TRIANGLES, visible=True, animation=False, position=None):
        '''
    ...
    def draw(self, Mp=poseMatrix()):
        ...
        if self.visible:
            ...
            # If the model has their animation True then do their animation. This is used to
            # rotate the trexes and pterodactyles every time they are drawn
            if self.animation:
                self.animate()
    ...
    def animate(self, keyPressed=None):

        # If no key was pressed we just do the constant animation that rotates the trexes and the pterodactyl

        if not keyPressed:
            if self.name == "trex":
                # By multiplying the matrixes in this order we rotate the trex around its own axis by 1 thousandth of a 
                # radian every time it is drawn to simulate a smooth roation
                self.M = np.matmul(self.M, rotationMatrixY(np.pi/1000))

            if self.name == "pterodactyl":
                # By multiplying the matrixes in this order we rotate the pterodactyl around the origin by -1 thousandth of a 
                # radian every time it is drawn to simulate a smooth roation
                self.M = np.matmul(rotationMatrixY(-np.pi/1000), self.M)

        elif keyPressed:
            if keyPressed == "forward":
                if self.name == "tri":
                    # Translates all the triceratops 0.2 forward in the z axis every time the up key is pressed
                    self.M = np.matmul(self.M, translationMatrix([0,0,0.2]))
                elif self.name == "phonebox":
                    # Rotates the phoneboxes by -1 20th of a radian about their own z axis to simulate them falling on the floor
                    self.M = np.matmul(self.M, rotationMatrixZ(-np.pi/20))
                elif self.name == "phonebox2":
                    # rotates the other phone boxes by 1 20th of a radian then translates them +6 in the y axis and -2 
                    # in the z axis to simulate them flying through the air
                    matrix = np.matmul(translationMatrix([0,6,-2]), rotationMatrixX(np.pi/20))
                    self.M = np.matmul(self.M, matrix)
            
            elif keyPressed == "backward":
                if self.name == "tri":
                    # Translates all the triceratops -0.2 backward in the z axis every time the down key is pressed
                    self.M = np.matmul(self.M, translationMatrix([0,0,-0.2]))
                elif self.name == "phonebox":
                    # Rotates the phoneboxes by 1 20th of a radian about their own z axis to return them to their standing position
                    self.M = np.matmul(self.M, rotationMatrixZ(np.pi/20))
                elif self.name == "phonebox2":
                    #Does the inverse operation by translating them -6 in the y axis and +2 in the z axis and then roating them - 1 
                    # 20th of a radian about their x axis to return them to their standing position
                    matrix = np.matmul(rotationMatrixX(-np.pi/20), translationMatrix([0,-6,2]))
                    self.M = np.matmul(self.M, matrix)
...
class DrawModelFromMesh(BaseModel):
    ...
    def __init__(self, scene, M, mesh, name=None, shader=None, visible=True,animation=False,position=None):
        '''
        Initialises the model data
        '''

        BaseModel.__init__(self, scene=scene, M=M, mesh=mesh, visible=visible, animation=animation, position=position)
