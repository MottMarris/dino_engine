...
class Scene:
    ...
    def __init__(self, width=800, height=600, shaders=None):
        ...
        # Controls how many animations of the triceratops are allowed. When its at 0, they
        # can't go backwars. When it's at 10, they can't go forwards
    ...
    def keyboard(self, event):
        ...
        # When up key is pressed, triceratops and phoneboxes with animation 
        # True will be animated forwards
        
        elif event.key == pygame.K_UP:
            if self.animationForward < 10:
                self.animationForward += 1
                for model in self.models:
                    if model.animation is True:
                        model.animate("forward")

        # When down key is pressed, triceratops and phoneboxes with animation 
        # True will be animated backwards
                        
        elif event.key == pygame.K_DOWN:
            if self.animationForward > 0:
                self.animationForward -= 1
                for model in self.models:
                    if model.animation is True:
                        model.animate("backward")
